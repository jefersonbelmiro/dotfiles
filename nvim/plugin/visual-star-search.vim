" From http://got-ravings.blogspot.com/2008/07/vim-pr0n-visual-search-mappings.html

" makes * and # work on visual mode too.
function! s:VSetSearch(cmdtype)
  let temp = @s
  norm! gv"sy
  let s:cache = '\V' . substitute(escape(@s, a:cmdtype.'\'), '\n', '\\n', 'g')
  let @s = temp
endfunction

"xnoremap * :<C-u>call <SID>VSetSearch('/')<CR>/<C-R>=@/<CR><CR> NN
"xnoremap # :<C-u>call <SID>VSetSearch('?')<CR>?<C-R>=@/<CR><CR>
vnoremap <silent> * :<C-u>call <SID>VSearch('/')<cr>
vnoremap <silent> # :<C-u>call <SID>VSearch('?')<cr>

nmap * viw:<C-u>call <SID>VSearch('/')<cr>
nmap # viw:<C-u>call <SID>VSearch('?')<cr>

function! s:VSearch(cmdtype)
    try 
        call s:VSetSearch(a:cmdtype)
        call feedkeys("b/" . s:cache . "\<cr>")
    catch /E385:/
        echo "eita"
    endtry
endfunction

" recursively vimgrep for word under cursor or selection if you hit leader-star
"nmap <leader>* :execute 'noautocmd vimgrep /\V' . substitute(escape(expand("<cword>"), '\'), '\n', '\\n', 'g') . '/ **'<CR>
"vmap <leader>* :<C-u>call <SID>VSetSearch()<CR>:execute 'noautocmd vimgrep /' . @/ . '/ **'<CR>

" dois clicks adiciona palavra a busca 
" nnoremap <silent> <2-LeftMouse> :let @/='\V\<'.escape(expand('<cword>'), '\').'\>'<cr>:set hls<cr> viw

" Destaca a palava clicada 2x
" para limpar: match none
"nnoremap <silent> <2-leftMouse> :exe 'highlight DoubleClick ctermbg=green guibg=green<bar>match DoubleClick /\V\<'.escape(expand('<cword>'), '\').'\>/'<cr>
